...
#
#   /mayhem/fuzz_config_decoder    config.FuzzConfigDecoder    (plumbing/format/config)
#   /mayhem/fuzz_packfile_parser   packfile.FuzzPackfileParser (plumbing/format/packfile)
#   /mayhem/fuzz_packfile_scanner  packfile.FuzzPackfileScanner(plumbing/format/packfile)
#   /mayhem/fuzz_object_commit     object.FuzzCommitDecode     (plumbing/object)
#   /mayhem/fuzz_index_decoder     index.FuzzIndexDecoder      (plumbing/format/index)
#
...
# Build-time LSan-off hook (SPEC.md 6.1, mayhem/lsan_off.c), linked into every libFuzzer binary AND
# standalone reproducer. Replaces a forbidden runtime ASan default-options override; ASan
...
$CC ${GO_DEBUG_FLAGS} ${SANITIZER_FLAGS:-} -c "$SRC/mayhem/lsan_off.c" -o "$LSAN_OBJ"
...
build_one() {
  ...
  $CXX $GO_DEBUG_FLAGS $SANITIZER_FLAGS $LIB_FUZZING_ENGINE "$ar" "$LSAN_OBJ" -o "/mayhem/${out}"
  ...
  if $CXX $GO_DEBUG_FLAGS ${SANITIZER_FLAGS:-} "$SRC/mayhem-build/standalone_main.o" "$ar" "$LSAN_OBJ" -lm \
...
# Fuzz func names below were bumped by an upstream sync that renamed them to be unique across
# packages: config.FuzzDecoder -> FuzzConfigDecoder, packfile.FuzzParser -> FuzzPackfileParser,
# packfile.FuzzScanner -> FuzzPackfileScanner, index.FuzzDecoder -> FuzzIndexDecoder.
build_one fuzz_config_decoder   plumbing/format/config   decoder_test.go        FuzzConfigDecoder
build_one fuzz_packfile_parser  plumbing/format/packfile parser_fuzz_test.go    FuzzPackfileParser
build_one fuzz_packfile_scanner plumbing/format/packfile scanner_fuzz_test.go   FuzzPackfileScanner
build_one fuzz_object_commit    plumbing/object          decode_fuzz_test.go    FuzzCommitDecode
build_one fuzz_index_decoder    plumbing/format/index    decoder_fuzz_test.go   FuzzIndexDecoder
