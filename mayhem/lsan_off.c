/* Fleet policy (SPEC.md 6.1): disable LeakSanitizer preventively at BUILD time. ASan
   ...
   go-git is pure Go, built with go-fuzz-build/go-118-fuzz-build into an archive that is then linked
   with clang++ under ASan. The Go runtime's allocations are not visible to LSan as freed, so LSan
   reports on the runtime itself rather than on anything the harness found.
